...
impl Web {
    ...
    async fn kasa_handler(
        Query(query): Query<KasaRouteQuery>,
        kasa_subscribers: Arc<RwLock<HashMap<String, Arc<RwLock<Subscriber>>>>>,
        topic: String,
    ) -> impl IntoResponse {
            ...
            router = router
                .route(
                    ...
                    routing::get({
                        ...
                        |query: Query<KasaRouteQuery>| {
                            Self::kasa_handler(query, kasa_subscribers, topic)
                        }
                    }),
                )
                .layer(
